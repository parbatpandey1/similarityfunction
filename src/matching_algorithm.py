...
✅ Rich matched dataset with EXACT matched skill pair
✅ Full profiles for VectorBridge email generation
"""
import pandas as pd
import numpy as np
from utils import *
...
def get_top_n_recommendations(dedup_df, n=TOP_N_MATCHES):
    """Top N mentor recommendations per mentee (mentee-centric)"""
    print(f"\n🏆 Top {n} mentors per mentee...")
    recommendations = []

    for mentee_id in dedup_df['mentee_person_id'].unique():
        mentee_matches = dedup_df[
            dedup_df['mentee_person_id'] == mentee_id
        ].sort_values('final_similarity_score', ascending=False).head(n)

        for rank, (_, match) in enumerate(mentee_matches.iterrows(), 1):
            recommendations.append({**match.to_dict(), 'recommendation_rank': rank})
...
def get_top_n_per_mentor(dedup_df, n=TOP_N_MATCHES):
    """Top N mentee recommendations per mentor (mentor-centric / reciprocal)"""
    print(f"\n🏆 Top {n} mentees per mentor...")
    recommendations = []

    for mentor_id in dedup_df['mentor_person_id'].unique():
        mentor_matches = dedup_df[
            dedup_df['mentor_person_id'] == mentor_id
        ].sort_values('final_similarity_score', ascending=False).head(n)
...
def build_rich_matched_dataset(assignments_df, mentors_df, mentees_df):
    """
    Build rich dataset for VectorBridge email generation.
    Contains:
    - EXACT matched skill pair (which combination won the match)
    - Full mentee profile (all interests, levels, aspirations)
    - Full mentor profile (all expertise, work, experience, contact)
    - Match scores + quality label
    """
    print("\n📋 Building rich matched dataset...")

    # ── Mentee profile columns ──────────────────────────────────
    mentee_profile_cols = {
    ...
    # ── Score + matched skill pair columns ──────────────────────
    # mentee_skill_text / mentor_skill_text = exact text that matched
    # mentee_skill_area / mentor_skill_area = 'main' or 'additional'
    score_cols = [
        'mentee_person_id',       'mentor_person_id',
        'final_similarity_score', 'skill_score',
        'aspiration_score',       'semantic_similarity',
        'expertise_gap',          'stream_bonus',
        'mentee_stream',          'mentor_stream',

        # ✅ EXACT matched skill pair — the winning combination
    ...
    rich_df = rich_df.sort_values(
        'final_similarity_score', ascending=False
    ).reset_index(drop=True)

    print(f"  ✅ Rich dataset: {len(rich_df)} matched pairs × {len(rich_df.columns)} columns")
    print(f"  ✓ Matched pair columns: "
          f"{'mentee_skill_text' in rich_df.columns and 'mentor_skill_text' in rich_df.columns}")
    return rich_df
...
def main():
    ...
    try:
        ...
        # ── Preview top 5 ───────────────────────────────────────
        print(f"\n🎯 TOP 5 ASSIGNMENTS:")
        for _, row in rich_df.head(5).iterrows():
            m_area = str(row.get('mentee_skill_area', '?')).replace('_', ' ')
            n_area = str(row.get('mentor_skill_area', '?')).replace('_', ' ')
            print(f"  {str(row['mentee_name'])[:20]:20} ↔ {str(row['mentor_name'])[:20]:20}"
                  f"  score: {row['final_similarity_score']:.4f}"
                  f"  [{m_area}] ↔ [{n_area}]")

        print("\n✅ MATCHING COMPLETE")
        print("  📄 rich_matched_dataset.csv")
        print("     ├─ mentee_skill_area / mentee_skill_text  ← exact matched skill")
        print("     ├─ mentor_skill_area / mentor_skill_text  ← exact matched skill")
        print("     ├─ full mentee profile (interests, aspirations)")
        print("     └─ full mentor profile (expertise, work, contact)")
